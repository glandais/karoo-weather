...
import io.github.glandais.karoo.weather.datatypes.views.WeatherNowView
...
/**
 * `weather-now` — the WMO icon and the temperature, with wind, feels-like and precipitation
 * probability added as the grid size allows (DESIGN §3.1).
 *
 * Its header earns its row, so this is the one graphical field with `showHeader = true`.
 */
@OptIn(ExperimentalGlanceRemoteViewsApi::class)
class WeatherNowDataType(private val context: Context, private val repo: WeatherRepository) :
    DataTypeImpl(DataTypeIds.EXTENSION, DataTypeIds.WEATHER_NOW) {

    /** Air temperature, °C — canonical SI, so other extensions can consume this stream. */
    ...
    override fun startView(context: Context, config: ViewConfig, emitter: ViewEmitter) {
        ...
        // Per-view state, never a property of this shared DataTypeImpl (ARCHITECTURE §4.3).
    ...
    private fun streamState(snapshot: WeatherSnapshot, nowSec: Long): StreamState {
        ...
        return StreamState.Streaming(
            DataPoint(dataTypeId, mapOf(DataType.Field.SINGLE to sample.temp))
        )
    }
    ...
    /** The hourly entries at or after `nowSec`, for the `(60,60)` outlook strip. */
    private fun outlookOf(snapshot: WeatherSnapshot, nowSec: Long): List<WeatherSample> {
        val hourly = snapshot.bundle?.here?.hourly ?: return emptyList()
        return hourly.filter { it.time >= nowSec - HOUR_SEC }
    }
